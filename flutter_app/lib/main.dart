...
class _MiqraAppState extends State<MiqraApp> {
  ...
  Future<void> _loadThemePreference() async {
    ...
    final saved = prefs.getBool('miqra_dark_mode');
    if (!mounted || saved == null) return;
    setState(() => _themeMode = saved ? ThemeMode.dark : ThemeMode.light);
  }

  Future<void> _toggleTheme() async {
    final dark = WidgetsBinding.instance.platformDispatcher.platformBrightness == Brightness.dark;
    final current = _themeMode == ThemeMode.dark || (_themeMode == ThemeMode.system && dark);
    final next = current ? ThemeMode.light : ThemeMode.dark;
    setState(() => _themeMode = next);
    final prefs = await SharedPreferences.getInstance();
    await prefs.setBool('miqra_dark_mode', next == ThemeMode.dark);
  }
...
class _AppShellState extends State<AppShell> {
  int index = 0;
  void _navigate(int v) => setState(() => index = v);
