...
class AudioPackButton extends StatefulWidget {
  ...
  @override
  State<AudioPackButton> createState() => _AudioPackButtonState();
}
...
class _AudioPackButtonState extends State<AudioPackButton> {
  ...
  void didUpdateWidget(covariant AudioPackButton oldWidget) {
    ...
    if (oldWidget.reciter.id != widget.reciter.id || oldWidget.surah.number != widget.surah.number) _refresh();
  }
  ...
  Future<void> _download() async {
    ...
    try {
      await AudioPackService.downloadSurah(
        reciterId: widget.reciter.id,
        surah: widget.surah.number,
        sourcesByAyah: sources,
        onProgress: (done, total) { if (mounted) setState(() => _done = done); },
      );
  ...
  Widget build(BuildContext context) {
    final unsupported = Theme.of(context).platform == TargetPlatform.macOS && false;
    ...
    return Column(
      ...
      children: [
        OutlinedButton.icon(
          ...
          icon: _busy
              ? SizedBox(width: 18, height: 18, child: CircularProgressIndicator(value: widget.surah.ayahs.isEmpty ? 0 : _done / widget.surah.ayahs.length, strokeWidth: 2))
              : Icon(_downloaded ? Icons.offline_pin_rounded : Icons.download_rounded),
